...
import RangesTab from "./tabs/RangesTab.jsx";
...
import { STATS_DEFAULT, loadStats, saveStats, loadHistory, calcPokerIQ, buildDailyProgram } from "./stats.js";
import { SPOTS, POKER_EVENTS, LEXIQUE, PROS, MENTAL_CONTENT, ARTICLES } from "./data/content.js";
...
function hapticsEnabled(){try{return localStorage.getItem("pf_haptics")!=="off";}catch{return true;}}
function vibrate(p){try{if(hapticsEnabled()&&navigator.vibrate)navigator.vibrate(p);}catch{}}
const VIB={tap:8,ok:[15,30,15],err:[60,40,60],next:12,win:[20,40,20,40,80]};
...
function RangeGrid({pos,action,stackBB,label,spot,showToggle=false,numTables=1,onOpenSolver}){
...
═══════════════════════════════════════ */
function RangePopup({heroPos,vilPos,heroAction,stackBB,onClose}){
...
function loadHands(){try{const s=localStorage.getItem("pf_hands");return s?JSON.parse(s):[];}catch{return [];}}
function saveHands(h){try{localStorage.setItem("pf_hands",JSON.stringify(h.slice(0,100)));}catch{}}
...
function loadHandsSafe(){try{return safeLoad("pf_hands_s",[]);}catch{return [];}}
function saveHandsSafe(h){safeStore("pf_hands_s",h.slice(0,100));saveHands(h);}
function loadStatsSafe(){
  const d=safeLoad("pf_stats_s",null);
  return d?{...STATS_DEFAULT,...d}:loadStats();
}
function saveStatsSafe(st){safeStore("pf_stats_s",st);saveStats(st);}
...
═══════════════════════════════════════ */
function SingleTable({spot,unit,numTables,showSol,sidebarCollapsed=false,trainerMode="gto",trainMode="spot",platform="pokerstars",onAnswer,onNext,isLast,onGoSolver,onFocusToggle,focusMode=false,chipTheme="blue",onToggleSol,onTableSettled,timerSec=20,field="Standard",coachLevel="Intermédiaire"}){
...
function TrainerTab({unit,onGoSolver:onGoSolverProp,chipTheme="blue",seed=null,onSeedApplied}){
...
function ReplayerTab({unit,onGoTrainer,onGoCoach,onGoRanges,initialText,onInitialApplied,initialTab="replay",onInitialTabApplied}){
...
function CoachFloatingButton({tab,onGoCoach}){
...
function SharkSolverTab({initialScenario=null,onGoTrainer=null,onGoReplayer=null,onInitialApplied=null}={}){
